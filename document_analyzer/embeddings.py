...
def analyze_semantic_similarity(
    ...
    try:
        not_in_use_docs = validate_list_input(
            ...
            "not_in_use_docs",
            min_length=1,
            item_validator=lambda p: validate_file_path(p, must_exist=True)
        )
    ...
    logger.info(
        f"Analyzing semantic similarity: {len(not_in_use_docs)} not_in_use vs "
        f"{len(active_docs)} active docs"
    )
    ...
    try:
        not_in_use_emb = model.encode(
            list(not_in_use_files.values()),
            convert_to_tensor=True,
            show_progress_bar=False
        )
        ref_emb = model.encode(
            list(reference_files.values()),
            convert_to_tensor=True,
            show_progress_bar=False
        )
    except Exception as e:
    ...
    logger.info(
        f"Found {len(results)} matches above threshold {threshold:.2f}"
    )
...
def analyze_active_document_similarities(
    ...
    logger.info(
        f"Analyzing similarities among {len(active_docs)} active documents..."
    )
...
def find_embeddings_clusters(
    ...
    if similarity_matrix.shape[0] != similarity_matrix.shape[1]:
        raise ValueError(
            f"Matrix must be square. Got shape {similarity_matrix.shape}"
        )
    ...
    for doc in document_names:
        ...
        if len(cluster_members) > 1:
            ...
            logger.debug(
                f"Found cluster with {len(cluster_members)} members"
            )
        else:
            visited.add(doc)

    logger.info(
        f"Found {len(clusters)} clusters with threshold {threshold:.2f}"
    )
    return clusters
