...
def find_active_documents(
    ...
    try:
        file_pattern = validate_string_input(
            file_pattern, 
            "file_pattern",
            max_length=50,
            pattern=r'^[\w\*\.\-]+$'
    ...
    if verbose:
        logger.info(
            f"Found {len(active_docs)} active documents in {root_dir}"
        )
        logger.info(
            f"Excluded patterns: {', '.join(exclude_patterns[:5])}..."
        )
    
    return active_docs


def find_not_in_use_documents(
    root_dir: Optional[Union[str, Path]] = None,
    file_pattern: str = MARKDOWN_FILE_PATTERN
) -> List[Path]:
    """
