...
class StructuralSoundnessChecker:
    ...
    def find_adr_and_architecture_files(self) -> Tuple[List[Path], List[Path]]:
        ...
        if adr_dir.exists():
            for file in adr_dir.rglob("*.md"):
                if file.name != "template.md":  # Exclude template
    ...
    def generate_soundness_report(self):
        """Generate comprehensive structural soundness report."""
        print("=" * 80)
        print("🏗️  STRUCTURAL SOUNDNESS ANALYSIS")
        print("=" * 80)
        print()
        
        # 1. Check ADR and Architecture citations
        print("1️⃣ CHECKING ADR & ARCHITECTURE CITATIONS")
        print("-" * 50)
        ...
        if not all_structural_files:
            print("❌ No ADR or architecture files found")
        else:
            print(f"📁 Found {len(adr_files)} ADR files and {len(arch_files)} architecture files")
            
            # Check citations in DOCUMENTS.md
            if not self.documents_md_path.exists():
                print("❌ DOCUMENTS.md not found - cannot verify citations")
            else:
                ...
                print(f"✅ Properly cited: {cited_count}/{len(citation_results)} files")
                print(f"❌ Missing citations: {missing_count} files")
        ...
        print("2️⃣ CHECKING TEMPLATE-TO-OUTPUT MAPPINGS")
        print("-" * 50)
        ...
        if not template_files:
            ...
            if total_expected > 0:
                ...
                print(f"📊 Generation rate: {total_actual}/{total_expected} ({generation_rate:.1f}%)")
            else:
                print("⚠️  No expected outputs found in templates")
                
            print(f"📝 Total generation instructions: {sum(len(mapping.instructions) for mapping in template_mappings)}")
            print(f"⚙️  Total generation commands: {sum(len(mapping.generation_commands) for mapping in template_mappings)}")
            ...
            for mapping in template_mappings:
                ...
                if mapping.expected_outputs:
                    print(f"   Expected outputs: {len(mapping.expected_outputs)}")
                    print(f"   Actual outputs: {len(mapping.actual_outputs)}")
                    ...
                    if mapping.actual_outputs:
                        ...
                        if len(mapping.actual_outputs) > 3:
                            print(f"      ... and {len(mapping.actual_outputs) - 3} more")
                    ...
                    if missing_outputs:
                        ...
                        if len(missing_outputs) > 3:
                            print(f"      ... and {len(missing_outputs) - 3} more")
                            
                    if mapping.generation_commands:
                        print("   🔧 Generation commands found:")
                        for cmd in mapping.generation_commands[:2]:
                            print(f"      - {cmd[:60]}...")
                else:
                    print("   ⚠️  No expected outputs identified")
        print()
        
        # 3. Summary and recommendations
        print("=" * 80)
        print("📊 STRUCTURAL SOUNDNESS SUMMARY")
        print("=" * 80)
        
        # Calculate overall scores
        citation_score = 0
        if all_structural_files:
            citation_results = self.check_citations_in_documents_md(all_structural_files) if self.documents_md_path.exists() else []
            if citation_results:
                cited_count = sum(1 for result in citation_results if result.cited_in_documents)
                citation_score = (cited_count / len(citation_results)) * 100
        ...
        if template_files:
            ...
            total_actual = sum(len(mapping.actual_outputs) for mapping in template_mappings)
            if total_expected > 0:
                generation_score = (total_actual / total_expected) * 100
                
        print(f"🏗️  Citation Coverage: {citation_score:.1f}%")
        print(f"⚙️  Template Generation: {generation_score:.1f}%")
        print(f"📁 ADR Files: {len(adr_files)}")
        print(f"🏛️  Architecture Files: {len(arch_files)}")
        ...
        if overall_score >= 80:
            ...
            print("   Strong structural soundness with good documentation coverage")
        elif overall_score >= 60:
            print(f"\n⚠️  Overall Assessment: GOOD ({overall_score:.1f}%)")
            print("   Solid foundation with some areas for improvement")
        else:
            print(f"\n❌ Overall Assessment: NEEDS ATTENTION ({overall_score:.1f}%)")
            print("   Structural gaps need addressing")
            
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        if citation_score < 80:
            print("   - Add missing ADR/architecture references to DOCUMENTS.md")
        if generation_score < 80:
            print("   - Implement missing template outputs or update template specifications")
        if len(adr_files) == 0:
            print("   - Create ADR files for major architectural decisions")
        if len(template_files) > 0 and generation_score > 0:
            print("   - Consider automating template-to-output generation")
