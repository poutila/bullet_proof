...
class InstructionPathTracer:
    ...
    def extract_document_info(self, doc_path: Path) -> InstructionNode:
    ...
    def normalize_path(self, path: str, from_dir: Path) -> Optional[Path]:
    ...
    def trace_from_document(self, start_path: Path, max_depth: int = 5) -> InstructionNode:
        """Trace instruction paths starting from a document."""
        queue = deque([(start_path, 0, None)])
        root_node = None
        nodes_by_path = {}
    ...
    def check_coverage(self, root_node: InstructionNode) -> Dict[str, List[str]]:
        ...
        def traverse(node: InstructionNode):
    ...
    def print_instruction_tree(self, node: InstructionNode, prefix: str = ""):
