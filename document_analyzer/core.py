...
from document_analyzer.validation import (
...
def split_sections(
    ...
    """Split markdown text into sections by paragraph.
...
def is_similar(
    ...
    try:
        ...
        logger.debug(
            f"Similarity check - set: {token_set_score}, sort: {token_sort_score}, "
...
def get_best_match_seq(
    ...
    # Validate inputs
    try:
        validated_section = validate_string_input(
            section,
            "section",
            max_length=100_000
        )
    ...
    for i, target in enumerate(targets):
        try:
            ...
            validated_target = validate_string_input(
                target,
                f"target[{i}]",
                max_length=100_000
            )
    ...
    logger.debug(
        f"Best match found with score {best_score:.3f} for section of length {len(section)}"
    )
...
def get_similarity_matrix(
    ...
    for i, text in enumerate(texts):
        try:
            validated = validate_string_input(
                text,
                f"text[{i}]",
                max_length=100_000
            )
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            try:
                ...
                logger.warning(
                    f"Error calculating similarity for texts {i} and {j}: {e}"
                )
                # Leave as 0.0 on error
    
    logger.info(
        f"Generated {n}x{n} similarity matrix with threshold {threshold:.2f}"
    )
...
def find_duplicate_groups(
    ...
    for i in range(n):
        ...
        if len(group) > 1:
            groups.append(group)
            logger.debug(
                f"Found duplicate group with {len(group)} documents: {group[:5]}..."
            )
    
    logger.info(
        f"Found {len(groups)} duplicate groups with threshold {threshold:.2f}"
    )
